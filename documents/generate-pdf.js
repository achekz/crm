...
// Ce script nécessite Node.js et Puppeteer
...
try {
    ...
    // Essayer d'installer Puppeteer
    const { execSync } = await import('child_process');
    try {
        execSync('npm install puppeteer', { stdio: 'inherit' });
        puppeteer = await import('puppeteer');
        console.log('✅ Puppeteer installé avec succès !');
    } catch (installError) {
        console.log('❌ Impossible d\'installer Puppeteer automatiquement.');
        console.log('💡 Veuillez exécuter manuellement: npm install puppeteer');
        process.exit(1);
    }
}
